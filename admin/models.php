...
$sql = "SELECT * FROM " . $tblcategory_models . " where " . $where;
$sql_limit = $sql . " LIMIT $from, " . $max_results;
$category_models = sql($sql_limit);
//pr($categories);
/**************************************************************************/
$total_results = sql("SELECT COUNT(*) as Num FROM " . $tblcategory_models . " where " . $where);

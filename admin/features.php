...
$sql = "SELECT * FROM " . $tblcategory_features . " where " . $where;
$sql_limit = $sql . " LIMIT $from, " . $max_results;
$category_features = sql($sql_limit);
//pr($categories);
/**************************************************************************/
$total_results = sql("SELECT COUNT(*) as Num FROM " . $tblcategory_features . " where " . $where);
...
<div class="row">
    <div class="col-md-12">
        <div class="card">
            <div class="row">
                <div class="col-md-6">
                    <h4 class="title">Manage Features of <a href="index.php?p=categories&id=<?= enc_password($cid); ?>"><?php echo $parent[0]['title_en'];?></a></h4>
                </div>
                <div class="col-md-6 add_new">
                    <a href="index.php?p=addeditfeatures&cid=<?= enc_password($pid); ?>">Add New feature</a>
                </div>
            </div>
            <div class="row">&nbsp;</div>
            <?php show_errors(); ?>
                ...
                <table class="table table-hover table-striped mytable">
                    ...
                    <tbody>
                        ...
                        if (count($category_features) > 0) {
                            foreach ($category_features as $v) {
                                ?>
                                <tr>
